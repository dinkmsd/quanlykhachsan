...
<div class="Module">
	...
	<div class="borderModule">		
		...
		<table width="450" border="0" cellpadding="5" cellspacing="0" style="margin:10px 0 0 50px;">
			...
			<tr>
				<td>{#security#} <label class="colorOrange">(*)</label></td>
				<td><img src="{$smarty.const.SITE_URL}lib/captcha/captcha.class.php" align="left" border="1"  style="margin-right:5px;" id="imgCaptcha" onclick="refreshImage(this)">
				<input type="hidden" name="hid_keycapcha" id="hid_keycapcha" value="{$smarty.session.key_captcha}">
				<input type="text" name="txt_captcha"  style="height:17px; width:100px;"/></td>
			</tr>
			<tr>
				<td>&nbsp;</td>
				<td><input type="submit" class="ButtonCart" style="border:none;" name="bt_contact" value="{#payment#}" />
				<input type="button"  class="ButtonCart" onclick="window.history.go(-1);" style="border:none;" name="bt_reset" value="{#cart#}" /></td>
			</tr>
		</table>				
		</form>
	</div>
